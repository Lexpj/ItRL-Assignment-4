...
class TomAndJerryEnvironmentTunnel:
    ...
    class Cat:
        def __init__(self, mid:tuple):
            ...
            """    
            self.mid = np.array(mid)
            self.time = np.random.randint(0,8) # Random number between 0 and 15
        ...
        def step(self,treats):
            ...
            if not self.eating:
                self.time = (self.time+self.speed+8)%8
            else:
                self.eating -= 1
    ...
    def step(self,a):
        ...
        for cat in self.cats:
            ...
            # if the mouse passes by the cat, its also terminal
            elif np.all(s_next == oldpos) and np.all(self.stateToPos(self.state) == pos):
                self.done = True
                r += self.rewards['cat']
                return s_next, r, self.done, info
